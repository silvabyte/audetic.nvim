---Voice command integration for audetic.nvim
...
show_feedback_window = function(title, lines, hl_group)
  ...
  if not feedback_buf or not vim.api.nvim_buf_is_valid(feedback_buf) then
    ...
    vim.api.nvim_set_option_value("filetype", "audetic_voice", { buf = feedback_buf })
  end
  ...
  -- Apply highlighting
  local ns = vim.api.nvim_create_namespace("audetic_voice_hl")
...
local function set_state(new_state, extra_info)
  ...
  elseif new_state == "recording" then
    ...
    vim.g.audetic_voice_status = "[REC]"
    ...
    client.send_message(session_id, prompt, { timeout = 600 }, function(msg_success, result)
      ...
      if api_error then
        ...
        vim.schedule(function()
          ...
          -- Clear after a delay
          local ui_cfg = config.get_ui() or {}
          vim.defer_fn(function()
            set_state("idle")
          end, ui_cfg.error_duration or 4000)
        end)
        return
      end
      ...
      vim.schedule(function()
        ...
        vim.defer_fn(function()
          set_state("idle")
        end, ui_cfg.success_duration or 2000)
      end)
    end)
  end)
end
...
function M.setup()
  ...
  vim.api.nvim_create_user_command("AudeticStatus", function()
    local state = M.get_state()
    vim.notify("Audetic Voice: " .. state, vim.log.levels.INFO)
